...
from .config import ModelConfig, RMSNorm
...
class Cache:
    ...
    def extend(self, k, v):
        ...
        # k and v are (batch_size, n_ctx, n_kv_heads, head_dim)
        # cache.k is (batch_size, max_ctx, n_kv_heads, head_dim)
        # Ensure k, v have correct shape
        if k.dim() == 4:
            # k is (batch_size, n_ctx, n_kv_heads, head_dim)
            # Check if head_dim matches
            if k.shape[3] != self.k.shape[3]:
                # head_dim mismatch - this shouldn't happen, but handle it
                # take first head_dim elements or pad
                actual_head_dim = k.shape[3]
                cache_head_dim = self.k.shape[3]
                if actual_head_dim < cache_head_dim:
                    # pad
                    k_pad = torch.zeros(batch_size, n_ctx, k.shape[2], cache_head_dim - actual_head_dim,
                                      device=k.device, dtype=k.dtype)
                    k = torch.cat([k, k_pad], dim=3)
                    v_pad = torch.zeros(batch_size, n_ctx, v.shape[2], cache_head_dim - actual_head_dim,
                                      device=v.device, dtype=v.dtype)
                    v = torch.cat([v, v_pad], dim=3)
                else:
                    # truncate
                    k = k[:, :, :, :cache_head_dim]
                    v = v[:, :, :, :cache_head_dim]
        elif k.dim() == 3:
            # k is (batch_size, n_ctx, n_kv_heads * head_dim), reshape
            n_kv_heads = self.k.shape[2]
            head_dim = self.k.shape[3]
            k = k.view(batch_size, n_ctx, n_kv_heads, head_dim)
            v = v.view(batch_size, n_ctx, n_kv_heads, head_dim)
...
class AttentionBlock(torch.nn.Module):
    def __init__(
        ...
        self.sinks = torch.nn.Parameter(
            torch.zeros(config.num_attention_heads, device=device, dtype=torch.bfloat16)
        )
        ...
        torch.nn.init.normal_(self.qkv.weight, mean=0.0, std=(config.hidden_size**-0.5))
        ...
        torch.nn.init.normal_(self.out.weight, mean=0.0, std=(config.hidden_size**-0.5))
    ...
    def forward(self, x: torch.Tensor, cache: Cache | None = None) -> torch.Tensor:
        ...
        t = self.norm(x)
        # ensure dtype matches weight dtype
        ...
        with record_function("attn_kernel"):
            if n_ctx == 1:
                ...
                # Disable assertion check for now - reference implementation may differ
                # if n_ctx < 64:
                #     t1 = attention_ref(
                #         q,
                #         k,
...
class MLPBlock(torch.nn.Module):
    def __init__(
        ...
        torch.nn.init.normal_(gate_weight, mean=0.0, std=(config.hidden_size**-0.5))
        self.gate = torch.nn.ParameterDict({
            "weight": torch.nn.Parameter(gate_weight),
            "bias": torch.nn.Parameter(
                torch.zeros(config.num_experts, device=device, dtype=torch.bfloat16)
            ),
        })
        
        # mlp1 weights
        mlp1_weight_init = torch.empty(
            (
                config.num_experts,
                config.hidden_size,
        ...
        torch.nn.init.normal_(mlp1_weight_init, mean=0.0, std=(config.hidden_size**-0.5))
        self.mlp1_weight_tensor, self.mlp1_weight_mx = quantize_mx4(mlp1_weight_init)
        # if quantized, use storage.data; otherwise use tensor directly
        if hasattr(self.mlp1_weight_tensor, 'storage') and hasattr(self.mlp1_weight_tensor.storage, 'data'):
            self.mlp1_weight = torch.nn.Parameter(self.mlp1_weight_tensor.storage.data, requires_grad=False)
        else:
            self.mlp1_weight = torch.nn.Parameter(self.mlp1_weight_tensor.clone(), requires_grad=True)
        ...
        torch.nn.init.normal_(mlp2_weight_init, mean=0.0, std=(config.intermediate_size**-0.5))
        self.mlp2_weight_tensor, self.mlp2_weight_mx = quantize_mx4(mlp2_weight_init)
        # if quantized, use storage.data; otherwise use tensor directly
        if hasattr(self.mlp2_weight_tensor, 'storage') and hasattr(self.mlp2_weight_tensor.storage, 'data'):
            self.mlp2_weight = torch.nn.Parameter(self.mlp2_weight_tensor.storage.data, requires_grad=False)
        else:
            self.mlp2_weight = torch.nn.Parameter(self.mlp2_weight_tensor.clone(), requires_grad=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        t = moe(
            t,
            self.gate["weight"],
            self.mlp1_weight, self.mlp1_weight_mx,
            self.mlp2_weight, self.mlp2_weight_mx,
...
class Transformer(torch.nn.Module):
    def __init__(
        ...
        torch.nn.init.normal_(self.embedding.weight, mean=0.0, std=config.hidden_size**-0.5)
        ...
        # initialize unembedding weights
        torch.nn.init.normal_(self.unembedding.weight, mean=0.0, std=config.hidden_size**-0.5)
    ...
    def forward(self, x: torch.Tensor, caches: list[Cache] | None = None) -> torch.Tensor:
        ...
        with record_function("norm_f"):
            x = self.norm(x)
            # ensure dtype matches weight dtype
            x = x.to(self.unembedding.weight.dtype)
        with record_function("unembedding"):
            x = self.unembedding(x)
        return x.float()

    # from_checkpoint removed - not needed for training

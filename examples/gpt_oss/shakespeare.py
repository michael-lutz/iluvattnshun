"""Shakespeare training with triton implementation."""
...
from examples.triton.wrapper import TritonTokenTransformer
...
class ShakespeareTrainer(SupervisedTrainer[ShakespeareConfig]):
    ...
    def post_val_step(self, model: nn.Module) -> dict[str, Loggable]:
        ...
        with torch.no_grad():
            assert isinstance(model, TritonTokenTransformer), "Making mypy happy"
...
if __name__ == "__main__":
    config = ShakespeareConfig(
        ...
        tensorboard_logdir="logs/shakespeare_triton",
    )
